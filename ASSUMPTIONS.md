...
| `MONEYNESS_SIGMA` | **2.0** | config | drop deep ITM/OTM: |moneyness − mean| > 2σ. The mean and σ are computed over the pooled 2022-2024 sample, so 2022 cleaning uses information from later years. Measured sensitivity (BL both-side): pooled, per-year and per-quote-date statistics give Sharpe 1.93/2.85/1.03, 1.92/2.85/1.03 and 1.93/2.86/1.04 with fills within 4, and dropping the filter entirely moves Sharpe by at most 0.23 (2.16/2.73/0.91), so the leakage does not affect results |
...
Alpha is statistically distinguishable from zero in only one of six model-years (BL 2023, t = 2.59); beta is small everywhere (|β| ≤ 0.04), and although BL 2023's is nominally significant (t = 3.2) it is economically negligible.
